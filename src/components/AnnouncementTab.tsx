...
export default function AnnouncementTab() {
  ...
  const handleGenerateTTS = async () => {
    if (!title.trim() || !message.trim()) {
      toast({ title: 'Data Tidak Lengkap', description: 'Judul dan Pesan wajib diisi untuk TTS', variant: 'destructive' });
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        toast({ title: '✅ Berhasil', description: 'Audio TTS disimpan di server' });
        setTitle('');
        setMessage('');
        loadAnnouncements(); 
      }
    } catch (error) {
      toast({ title: 'Error', description: 'Koneksi backend 8000 gagal', variant: 'destructive' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.type.startsWith('audio/')) {
      toast({ title: 'Error', description: 'File harus berupa audio MP3', variant: 'destructive' });
      return;
    }
    ...
    toast({ title: 'File Terpilih', description: 'Silakan isi Judul lalu klik Simpan ke Daftar' });
  };

  const handleUploadFinal = async () => {
    if (!title.trim()) {
      toast({ title: 'Judul Wajib Diisi', description: 'Berikan judul untuk file yang diupload', variant: 'destructive' });
      return;
    }
    ...
    try {
      const response = await fetch('http://localhost:8000/api/tts/upload', {
        method: 'POST',
        body: formData,
      });
      ...
      const result = await response.json();
      if (result.success) {
        toast({ title: '✅ Berhasil', description: 'File MP3 berhasil ditambahkan ke daftar' });
        setTitle('');
      ...
      toast({ title: 'Gagal Upload', description: 'Pastikan backend port 8000 menyala', variant: 'destructive' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      if (result.success) {
        toast({ title: '✅ Berhasil', description: 'Audio telah dihapus' });
        loadAnnouncements();
      } else {
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Generate audio dari teks atau upload file MP3</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-3">
            ...
            <div>
              <Label htmlFor="message">Pesan (Hanya untuk TTS)</Label>
              <Textarea
                id="message"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
              ...
              <div className="relative">
                ...
                />
                <Button
                  onClick={selectedFile ? handleUploadFinal : () => fileInputRef.current?.click()}
                  disabled={loading}
                ...
                >
                  ...
                  {selectedFile ? "Simpan ke Daftar" : "Upload MP3"}
                </Button>
              </div>
            </div>
            ...
            {previewAudio && (
              <div className="border-t pt-4">
                <Label className="mb-2 block font-bold text-emerald-700">Preview Audio</Label>
                <audio controls className="w-full" src={previewAudio}>
                  Browser Anda tidak mendukung audio player.
                </audio>
                {selectedFile && (
                   <p className="text-xs text-blue-600 mt-2 italic">* File siap diupload: {selectedFile.name}</p>
                )}
              </div>
            )}
          </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Semua pengumuman yang telah dibuat</CardDescription>
        </CardHeader>
        <CardContent>
                ...
                >
                  <div className="flex items-center space-x-4 flex-1">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-gray-600 mt-1">{item.message || "File Audio"}</p>
                      <p className="text-xs text-gray-400 mt-2">
                        {item.audio_type === 'generated' ? '🎙️ TTS Indonesia' : 'Uploaded MP3'} • {new Date(item.created_at).toLocaleString('id-ID')}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex space-x-2">
                    <Button 
                        size="sm" 
                        variant="outline"
                        className={playingId === item.id ? "bg-emerald-100 text-emerald-700 border-emerald-200" : ""}
                        onClick={() => handlePlayPause(item.id, item.audio_url)}
                      >
